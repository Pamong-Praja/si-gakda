...
type Parsed = {
  tanggal: string;
  waktuMulai: string;   // ← tambah
  waktuSelesai: string; // ← tambah
  lokasi: string;
  personel: string;
  uraian: string;
  dasar_hukum: string;
...
// 1. Deteksi rentang waktu: "Pukul 09:00 WIB - 14:30 WIB"
//    atau "09:00 - 14:30 WIB" atau "09.00 - 14.30 WIB"
const rangeMatch = text.match(/(?:Pukul|Jam|Waktu)\s*[:]?\s*(\d{1,2})[:.](\d{2})\s*(?:WIB|WITA|WIT)?\s*(?:[-–s/d]\s*)?(\d{1,2})[:.](\d{2})\s*(?:WIB|WITA|WIT)?/i);
if (rangeMatch) {
  // Pastikan yang ditangkap adalah angka, bukan kata
  const jam1 = parseInt(rangeMatch[1]);
  const menit1 = parseInt(rangeMatch[2]);
  if (!isNaN(jam1) && !isNaN(menit1) && jam1 >= 0 && jam1 <= 23 && menit1 >= 0 && menit1 <= 59) {
    waktuMulai = `${String(jam1).padStart(2, '0')}:${String(menit1).padStart(2, '0')}`;
  }
  if (rangeMatch[3] && rangeMatch[4]) {
    const jam2 = parseInt(rangeMatch[3]);
    const menit2 = parseInt(rangeMatch[4]);
    if (!isNaN(jam2) && !isNaN(menit2) && jam2 >= 0 && jam2 <= 23 && menit2 >= 0 && menit2 <= 59) {
      waktuSelesai = `${String(jam2).padStart(2, '0')}:${String(menit2).padStart(2, '0')}`;
    }
  }
}

// 2. Jika tidak ada rentang, coba deteksi 1 waktu: "Pukul 09:00 WIB"
if (!waktuMulai) {
  const singleMatch = text.match(/(?:Pukul|Jam|Waktu)\s*[:]?\s*(\d{1,2})[:.](\d{2})\s*(?:WIB|WITA|WIT)?/i);
  if (singleMatch) {
    const jam = parseInt(singleMatch[1]);
    const menit = parseInt(singleMatch[2]);
    if (!isNaN(jam) && !isNaN(menit) && jam >= 0 && jam <= 23 && menit >= 0 && menit <= 59) {
      waktuMulai = `${String(jam).padStart(2, '0')}:${String(menit).padStart(2, '0')}`;
    }
  }
}

// 3. Jika masih tidak ada, coba cari pola "09:00 WIB" (tanpa kata Pukul)
if (!waktuMulai) {
  const simpleMatch = text.match(/\b(\d{1,2})[:.](\d{2})\s*(?:WIB|WITA|WIT)?\b/);
  if (simpleMatch) {
    const jam = parseInt(simpleMatch[1]);
    const menit = parseInt(simpleMatch[2]);
    if (!isNaN(jam) && !isNaN(menit) && jam >= 0 && jam <= 23 && menit >= 0 && menit <= 59) {
      waktuMulai = `${String(jam).padStart(2, '0')}:${String(menit).padStart(2, '0')}`;
    }
  }
  ...
  return { tanggal, waktuMulai, waktuSelesai, lokasi, personel, uraian, dasar_hukum };
}
// ============================================================
...
export function SmartInput({ onCancel, onSaved }: Props) {
  ...
  function handleDetect() {
    ...
    setWaktuMulai(result.waktuMulai);   // ← tambah
    setWaktuSelesai(result.waktuSelesai); // ← tambah
    setDetected(true);
